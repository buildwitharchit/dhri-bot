...
from v4_legacy.bot.router import route_update
from config import ALL_SUBSKILLS, settings
from shared.db.client import close_db_pool, init_db_pool
...
async def cleanup_endpoint(request: Request) -> Response:
    ...
    from v4_legacy.handlers.session_cleanup import cleanup_stale_sessions
...
async def reports_endpoint(request: Request) -> Response:
    if request.headers.get("X-Admin-Secret") != settings.ADMIN_REPORTS_SECRET:
        return Response(status_code=403)
    from v4_legacy.handlers.weekly_reports import send_weekly_reports_to_all
